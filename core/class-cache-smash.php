...
class WPtouchCacheSmash {
	...
	public function is_wp_super_cache_broken() {
		return ( $this->is_wp_super_cache_configured() && defined( 'WP_CACHE') && WP_CACHE );
	}
	...
	private function find_in_array_no_case( $needle, $haystack_array ) {
		return in_array( strtolower( $needle ), array_map( 'strtolower', $haystack_array ) );
	}
	...
	private function is_wp_super_cache_configured() {
	...
	public function is_cache_configured() {
		...
		if ( $this->is_wp_super_cache_detected() ) {
			...
			if ( $super_cache_enabled ) {
				$cache_configured = $this->is_wp_super_cache_configured();
			} else {
