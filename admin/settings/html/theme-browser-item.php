	...
	<div class="item-information">
		<?php if ( wptouch_cloud_theme_update_available() ) { ?>
			<?php if ( !defined( 'WPTOUCH_IS_FREE' ) ) { ?>
				...
				<a class="btn btn-small btn-info upgrade" href="#" data-name="<?php wptouch_the_theme_base(); ?>" data-url="<?php wptouch_the_theme_download_url(); ?>" data-loading-text="<i class='icon-cloud-download'></i><?php _e( 'Upgrading...', 'wptouch-pro' ); ?>">
					<i class="icon-cloud-download"></i>
					<?php echo sprintf( __( 'Update to %s', 'wptouch-pro' ), wptouch_cloud_theme_get_update_version() ); ?>
				</a>
				<?php } else { ?>
				<a class="btn btn-small btn-info cant-upgrade" href="<?php wptouch_the_theme_download_url(); ?>"><i class="icon-cloud-download"></i> <?php echo sprintf( __( 'Download %s Update', 'wptouch-pro' ), wptouch_cloud_theme_get_update_version() ); ?></a>
				<?php } ?>
			<?php } ?>
		<?php } ?>
		<h4>
	...
	<ul class="item-actions">
				...
				<li>
					<?php if ( wptouch_theme_type() == 'WITHLICENSE' ) { ?>
						<?php if ( defined( 'WPTOUCH_IS_FREE' ) ) { ?>
							...
							<a class="btn btn-small" href="<?php echo wptouch_theme_info_url(); ?>"><?php _e( 'Available in WPtouch Pro', 'wptouch-pro' ); ?></a>
							<?php } ?>
						<?php } else { ?>
							<?php if ( wptouch_theme_info_url() ) { ?>
							<a class="btn btn-small" href="<?php echo wptouch_theme_info_url(); ?>"><?php _e( 'More Info', 'wptouch-pro' ); ?></a>
							<?php } ?>

							<?php if ( wptouch_has_license() ) { ?>
							<a class="btn btn-small buynow" href="<?php wptouch_the_theme_buy_url(); ?>"><?php _e( 'Upgrade License', 'wptouch-pro' ); ?></a>
							<?php } else { ?>
							<a class="btn btn-small buynow" href="<?php wptouch_the_theme_buy_url(); ?>"><?php _e( 'Get License', 'wptouch-pro' ); ?></a>
							<?php } ?>
						<?php } ?>

					<?php } else { ?>
						<a class="btn btn-small buynow" href="<?php wptouch_the_theme_buy_url(); ?>"><?php _e( 'Buy Now', 'wptouch-pro' ); ?></a>
					<?php } ?>
				</li>
			<?php } else { ?>
				<?php if ( current_user_can( 'install_plugins' ) ) { ?>
					<?php if ( !defined( 'WPTOUCH_IS_FREE' ) ) { ?>
						<li>
							<?php if ( wptouch_can_cloud_install( true ) ) { ?>
							<a class="btn btn-small btn-info download" href="#" data-name="<?php wptouch_the_theme_base(); ?>" data-url="<?php wptouch_the_theme_download_url(); ?>" data-loading-text="<i class='icon-cloud-download'></i><?php _e( 'Downloading...', 'wptouch-pro' ); ?>">
								<i class="icon-cloud-download"></i><?php _e( 'Install', 'wptouch-pro' ); ?>
							</a>
							<?php } else { ?>
							<a class="btn btn-small btn-info" href="<?php wptouch_the_theme_download_url(); ?>"><i class="icon-cloud-download"></i> <?php _e( 'Download', 'wptouch-pro' ); ?></a>
							<?php } ?>
						</li>
					<?php } ?>
				<?php } ?>
		...
		<?php } else { ?>
			...
			<?php if ( wptouch_is_theme_active() && current_user_can( 'manage_options' ) ) { ?>
				<li><a class="btn btn-small btn-success" href="admin.php?page=wptouch-admin-theme-settings"><?php _e( 'Setup', 'wptouch-pro' ); ?></a></li>			
			<?php } ?>
