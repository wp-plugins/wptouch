...
<?php if ( is_array( $settings->plugin_hooks ) && count( $settings->plugin_hooks ) ) { ?>
	...
	<?php foreach( $settings->plugin_hooks as $key => $value ) { ?>
		...
		<label for="<?php echo $key; ?>">
			...
			<i class="wptouch-tooltip icon-question-sign" data-original-title="<?php echo sprintf( __( 'When unchecked, %s will be disabled for users viewing your WPtouch Pro theme.', 'wptouch-pro' ), $friendly_name ); ?>"></i>
		</label>
		</li>
	<?php } ?>
		</ul>
