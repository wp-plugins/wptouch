...
<div id="comment_wrapper">
	...
	<div class="comwrap">
			<div class="comtop">		
				<?php if (bnc_is_gravatars_enabled()) { echo get_avatar( $comment, $size = '28', $default = '' . compat_get_plugin_url( 'wptouch' ) . '/themes/core/core-images/blank_gravatar.jpg' ); } ?>		
					<a href="<?php comment_author_url(); ?>"><?php comment_author(); ?></a>
			<div class="comnumber"><?php $i++; ?></div>  
			<div class="comdater"><?php $d = (time() - strtotime(get_comment_date('F jS, Y')))/(60*60*24); if ($d < 1) echo (' ' . comment_time('h:ia') . ' today'); else echo floor($d) . ' days ago'; ?></div>  
	</div><!--end comtop-->
...
<?php else : ?>
  <!--  Let's check for advanced JS setting, and if it's enabled do fancy ajax comments -->
	...
	<?php if (!function_exists('cas_register_post')) { ?>
		<div id="refresher" style="display:none">&raquo; <a href="#" onclick="this.location.reload(); return false;"><?php _e( "Refresh the page", "wptouch" ); ?></a> <?php _e( "to post a new comment.", "wptouch" ); ?></div>
