...
add_filter( 'wptouch_modify_setting__foundation__enabled_custom_post_types', 'wptouch_test', 1 );
function wptouch_test( $setting ) {
...
function wptouch_preserve_theme_custom_post_types() {
	...
	if ( $settings->enable_custom_post_types ) {
		$post_types_to_preserve = unserialize( stripslashes( $settings->enabled_custom_post_types ) );
		if ( is_array( $post_types_to_preserve ) ) {
			foreach ( $post_types_to_preserve as $type => $object ) {
				$post_types_to_preserve[ $type ] = urldecode( $object );
			}
...
function wptouch_register_theme_custom_post_types() {
	if ( wptouch_is_mobile_theme_showing() ) {
		...
		if ( is_array( $post_types_to_preserve ) && count( $post_types_to_preserve ) > 0 ) {
			...
			foreach ( $post_types_to_preserve as $type => $object ) {
				$object = unserialize( urldecode( $object ) );
...
function wptouch_custom_posts_add_to_search( $post_types ) {
	$settings = foundation_get_settings();
	$custom_post_types = unserialize( stripslashes( $settings->enabled_custom_post_types ) );
	...
	if ( ( is_single() && !is_page() ) || is_home() ) {
		...
		if ( wptouch_is_mobile_theme_showing() ) {
			...
			if ( $post_types && count( $post_types )  ) {
				$post_type_array = 	array_flip( unserialize( stripslashes( $settings->enabled_custom_post_types ) ) );
